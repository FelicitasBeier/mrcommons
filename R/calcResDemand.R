...
calcResDemand <- function(cellular = FALSE, scenario = "dafault") {
  ...
  biomass1 <- add_dimension(
    dimSums(collapseNames(
      calcOutput("ResBiomass",
        cellular = cellular, plantparts = "ag", aggregate = FALSE,
        scenario = scenario
      )[, , resCereals]
    ), dim = 3.1),
    add = "kres", nm = "res_cereals"
  )
  biomass2 <- add_dimension(
    dimSums(collapseNames(
      calcOutput("ResBiomass",
        cellular = cellular, plantparts = "ag", aggregate = FALSE,
        scenario = scenario
  ...
  material <- mbind(
    biomass[, , "res_cereals"] * (devStatePast * 0 + (1 - devStatePast) * 0.05),
  ...
  out <- mbind(
    add_dimension(production, dim = 3.1, nm = "production"),
